      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <section className="bg-white rounded-xl shadow p-6 flex flex-col">
          ...
          <ul className="space-y-2 text-gray-700">
            {temasEvaluar.map((tema) => (
              <li key={tema.id} className="flex items-center gap-4 pl-2 border-l-4 border-yellow-200">
                <span className="font-medium">{tema.tema}</span>
                <span className="text-yellow-700 font-bold">{tema.puntos} pts</span>
              </li>
            ))}
          </ul>
        </section>
